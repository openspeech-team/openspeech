...
from openspeech.models import ListenAttendSpellWithMultiHeadConfigs, ListenAttendSpellWithMultiHeadModel
...
class TestListenAttendSpellWithMultiHead(unittest.TestCase):
    def test_forward(self):
        configs = build_dummy_configs(
            model_configs=ListenAttendSpellWithMultiHeadConfigs(),
            criterion_configs=LabelSmoothedCrossEntropyLossConfigs(),
        )

        vocab = KsponSpeechCharacterTokenizer(configs)
        model = ListenAttendSpellWithMultiHeadModel(configs, vocab)
